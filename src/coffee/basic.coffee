...
$.fn.stickyfloat = (options) ->
    ...
    initFloat = ->
        ...
        bottomPos = if bottomPos < 0 then 0 else bottomPos
        ...
        if (pastStartOffset or objFartherThanTopPos) and objBiggerThanWindow
            newPos = if opts.stickToBottom then dom.scrollTop() + $(window.top).height() - $obj.outerHeight() - startOffset - opts.offsetY else dom.scrollTop() - startOffset + opts.offsetY

            if newPos > bottomPos
                newPos = bottomPos 

            else if dom.scrollTop() < opts.startOffset and not opts.stickToBottom
